...
export function AdminContentManagement({ connection, kind, onError }: Props) {
  ...
  function openAnnouncement(item?: PlatformAnnouncement) {
    ...
    setPublishStartAt(item?.publishStartAt?.slice(0, 16) ?? '')
    setPublishEndAt(item?.publishEndAt?.slice(0, 16) ?? '')
  }
  ...
  async function saveAnnouncement() {
    if (!connection || !title.trim() || !content.trim()) return
    setSaving(true)
    try {
      ...
      onError(caught instanceof Error ? caught.message : '플랫폼 공지를 저장하지 못했습니다.')
