...
void main() {
  ...
  test('session controller clears an expired session', () async {
    final now = DateTime.utc(2026, 7, 29, 12);
    ...
    final controller = SessionController(sessionStore: store, now: () => now);
  ...
  test('session controller exposes restore failure without hanging', () async {
    final controller = SessionController(sessionStore: _FailingSessionStore());
