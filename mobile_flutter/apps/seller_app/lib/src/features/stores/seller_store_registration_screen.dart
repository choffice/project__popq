...
class _SellerStoreRegistrationScreenState
          ...
          children: [
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                ...
                helperText: '이미지 업로드 API 연결 전까지 URL로 등록합니다.',
              ),
              validator: (value) {
          ...
          children: [
            ...
            Wrap(
              ...
              children: _days.entries.map((entry) {
                final selected = _closedDays.contains(entry.key);
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    enabled: !_submitting && _tags.length < 8,
                    maxLength: 30,
  ...
  void _addTag() {
    ...
    if (tag.isEmpty) {
      _showMessage('추가할 검색 키워드를 입력해 주세요.');
      return;
    }

    if (tag.length > 30) {
      _showMessage('검색 키워드는 30자 이하로 입력해 주세요.');
      return;
    }

    final duplicated = _tags.any(
          (existing) => existing.toLowerCase() == tag.toLowerCase(),
    );

    if (duplicated) {
      _showMessage('이미 추가한 검색 키워드예요.');
      return;
    }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await widget.selectionController.select(
        created.storeId,
      );
