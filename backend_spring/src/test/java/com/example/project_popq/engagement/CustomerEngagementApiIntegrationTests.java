...
class CustomerEngagementApiIntegrationTests {
    ...
    void customerManagesInterestsReviewAndProfile() throws Exception {
        ...
        mockMvc.perform(post(
                        ...
                        .content(reviewBody(5, "Not completed yet")))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.error.code")
                        .value("REVIEW_NOT_ALLOWED"));

        completeOrder(sellerToken, storeId, orderPublicId);

        String reviewResponse = mockMvc.perform(post(
                        "/api/v1/customer/reviews/orders/{orderPublicId}",
                        orderPublicId
                )
                        .header("Authorization", bearer(customerToken))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(reviewBody(5, "Excellent coffee")))
        ...
        mockMvc.perform(post(
                        ...
                        .header("Authorization", bearer(customerToken))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(reviewBody(4, "Duplicate")))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.error.code")
        ...
        mockMvc.perform(put(
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(reviewBody(1, "Unauthorized")))
        ...
        mockMvc.perform(put(
                        ...
                        .header("Authorization", bearer(customerToken))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(reviewBody(4, "Updated review")))
                .andExpect(status().isOk())
    ...
    private void completeOrder(
        ...
        transition(token, storeId, orderPublicId, "accept");
        transition(token, storeId, orderPublicId, "prepare");
        transition(token, storeId, orderPublicId, "ready");
        transition(token, storeId, orderPublicId, "complete");
    }
    ...
    private String reviewBody(int rating, String content) {
        return """
                {
                  "rating": %d,
                  "content": "%s"
